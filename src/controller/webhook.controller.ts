...
export class WebhookController {
    ...
    public static initialize(): WebhookController {
        if (WebhookController.instance) throw new Error('WebhookController est déjà initialisé');
    ...
    public static getInstance(): WebhookController {
        if (!WebhookController.instance) throw new Error('WebhookController non initialisé. Appelez initialize() d\'abord.');
        return WebhookController.instance;
    }
    ...
    async eventProcess(req: FastifyRequest, reply: FastifyReply) {
        ...
        try {
            ...
            if (!Security.verifyMessage(hmac, req.headers[Security.TWITCH_MESSAGE_SIGNATURE]! as string)) {
                return reply.code(403).send('Invalid security');
            }
        
            const user = await instance._twitchService!.api.users.getUserById((req.body as { event: { broadcaster_user_id: string } }).event.broadcaster_user_id);
        
            if (!user) return reply.send({error: 'unknown streamer'}).code(404);
                
            const streamer = instance._streamerController!.cache.find(
                (streamer) => streamer.name.toLowerCase() === user!.displayName.toLowerCase()
            );
        
            if (!streamer) return reply.send({error: 'streamer not part of the event'}).code(404);
                    
            switch ((req.body as { subscription: { type: string } }).subscription
                .type) {
            case 'stream.online':
